...
int computeFlagsAndAddress(symbol* symbolArray[], address* addresses, segment* segments, int format)
{
    ...
    {
        flagValues += (FLAG_N + FLAG_I);
        int len = strlen(buffer);
        char *test_X = strstr(buffer, INDEX_STRING);
        if(test_X != NULL)
        {
            flagValues += FLAG_X;
            memmove(buffer + len - 2, buffer + len, 2);
        }
    }
    if(format == FORMAT_4)
    {
    ...
    if(strcmp(segments -> operand,"RSUB") == 0)
    {
        flagValues *= FORMAT_3_MULTIPLIER;
        return flagValues;
    }

    int bufferVal = 0;
    if(isNumeric(buffer)) //determining if buffer value is integer
    {
        bufferVal= strtol(buffer,NULL,10);
        if(format == FORMAT_3)
        {
            flagValues *= FORMAT_3_MULTIPLIER;
        }
        if(format == FORMAT_4)
        {
            flagValues *= FORMAT_4_MULTIPLIER;
        }
        flagValues += bufferVal;
        return flagValues;
    }
    else
    {
        bufferVal = getSymbolAddress(symbolArray,buffer);
    }
    if(format == FORMAT_4)
    {
        flagValues *= FORMAT_4_MULTIPLIER;
    ...
    int sumAddy = addresses -> current + addresses -> increment;
    int displacement = getSymbolAddress(symbolArray,buffer)- sumAddy;
    ...
    {
        displacement = getSymbolAddress(symbolArray,buffer) - addresses -> base;
        ...
        else
        {
            ...
            //printf("%s ",buffer);
            //printf("%s ",segments->operation);
            displayError(ADDRESS_OUT_OF_RANGE,buffer);
            exit(1);
        }
    }
    if(displacement < 0)
...
int getRegisters(char* operand)
{
    int regVal = getRegisterValue(operand[0]);
    regVal *= REGISTER_MULTIPLIER;
    if(strlen(operand) > 1)
    {
        regVal += getRegisterValue(operand[strlen(operand) - 1]);
    }
    return regVal;
}
...
void performPass2(struct symbol* symbolTable[], char* filename, address* addresses)
{
	...
	// Place your code for the performPass2 function here
        ...
        if(isDirective(segments -> operation))
        {
            ...
            if(isBaseDirective(isDirective(segments -> operation)))
            {
            ...
            if(isReserveDirective(isDirective(segments -> operation)))
            {
                if(objectData.recordByteCount > 0)
                {
                    flushTextRecord(fileObj,&objectData,addresses);
            ...
            if(isDataDirective(isDirective(segments -> operation)))
            {
                addresses -> increment = getMemoryAmount(directiveType,segments -> operand);
                ...
                int value = getByteValue(directiveType,segments -> operand);
                objectData.recordEntries[objectData.recordEntryCount].numBytes = addresses -> increment;
                objectData.recordEntries[objectData.recordEntryCount].value = value;
                objectData.recordEntryCount++;
                objectData.recordByteCount += addresses -> increment;
                writeToLstFile(fileLst,addresses -> current,segments,value);
            }
        }
        if(isOpcode(segments -> operation))
        {
            int localOpcode = getOpcodeValue(segments -> operation);
            addresses -> increment = getOpcodeFormat(segments -> operation);
            if(addresses -> increment < 0)
            {
            ...
            switch(addresses->increment){
                case 1:
                    break;
                case 2:
                    localOpcode *= OPCODE_MULTIPLIER;
                    localOpcode += getRegisters(segments->operand);
                    break;
                case 3:
                    localOpcode *= OPCODE_MULTIPLIER * OPCODE_MULTIPLIER;
                    localOpcode += computeFlagsAndAddress(symbolTable, addresses, segments, FORMAT_3);
                    break;
                case 4:
                    localOpcode *= OPCODE_MULTIPLIER * OPCODE_MULTIPLIER * OPCODE_MULTIPLIER;
                    localOpcode += computeFlagsAndAddress(symbolTable, addresses, segments, FORMAT_4);
                    break;
            }
            if(objectData.recordByteCount > (MAX_RECORD_BYTE_COUNT - addresses -> increment))
            {
            ...
            objectData.recordEntries[objectData.recordEntryCount].numBytes = addresses -> increment;
            objectData.recordEntries[objectData.recordEntryCount].value = localOpcode;
            objectData.recordEntryCount++;
            objectData.recordByteCount += addresses -> increment;
            writeToLstFile(fileLst,addresses -> current,segments,localOpcode);
        }
        addresses -> current += addresses -> increment;
